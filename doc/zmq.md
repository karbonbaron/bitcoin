...
The Monicoin Core daemon can be configured to act as a trusted "border
router", implementing the monicoin wire protocol and relay, making
...
The ZeroMQ feature in Monicoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building monicoind:
...
For instance:
    ...
    $ monicoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/monicoind.tx.raw \
...
These options can also be provided in monicoin.conf.
...
From the perspective of monicoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into monicoind directly. Furthermore, no information is
...
using. Monicoind appends an up-counting sequence number to each
