...
Syntax: `getcoins.py [-h|--help] [-c|--cmd=<monicoin-cli path>] [-f|--faucet=<faucet URL>] [-a|--addr=<signet bech32 address>] [-p|--password=<faucet password>] [--] [<monicoin-cli args>]`

* `--cmd` lets you customize the monicoin-cli path. By default it will look for it in the PATH
* `--faucet` lets you specify which faucet to use; the faucet is assumed to be compatible with https://github.com/kallewoof/monicoin-faucet
* `--addr` lets you specify a Signet address; by default, the address must be a bech32 address. This and `--cmd` above complement each other (i.e. you do not need `monicoin-cli` if you use `--addr`)
...
To mine the first block in your custom chain, you can run:
    ...
    cd src/
    CLI="./monicoin-cli -conf=mysignet.conf"
    MINER="..contrib/signet/miner"
    GRIND="./monicoin-util grind"
