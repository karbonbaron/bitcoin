This directory contains the MonicoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Monicoin UI.
...
ClientModel is used by the main application `monicoingui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a monicoin:... link from a browser).
...
* UI elements like MonicoinAmountField, which inherit from QWidget.
* `monicoinstrings.cpp`: automatically generated
* `monicoinunits.(h/cpp)`: MC / mMC / etc handling
* `callback.h`
...
* don't change `local/monicoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
4. Enter "monicoin-qt" as project name, enter src/qt as location
5. Leave the file selection as it is
6. Confirm the "summary page"
7. In the "Projects" tab select "Manage Kits..."
8. Select the default "Desktop" kit and select "Clang (x86 64bit in /usr/bin)" as compiler
9. Select LLDB as debugger (you might need to set the path to your installation)
10. Start debugging with Qt Creator (you might need to the executable to "monicoin-qt" under "Run", which is where you can also add command line arguments)
