...
#include <qt/monicoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Monicoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new MonicoinAddressEntryValidator(parent));
    widget->setCheckValidator(new MonicoinAddressCheckValidator(parent));
}

bool parseMonicoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no monicoin: URI
    if(!uri.isValid() || uri.scheme() != QString("monicoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!MonicoinUnits::parse(MonicoinUnits::MC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseMonicoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseMonicoinURI(uriInstance, out);
}

QString formatMonicoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("monicoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MonicoinUnits::format(MonicoinUnits::MC, info.amount, false, MonicoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openMonicoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", MONICOIN_CONF_FILENAME));
    ...
    /* Open monicoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Monicoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Monicoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Monicoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Monicoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "monicoin.desktop";
    return GetAutostartDir() / strprintf("monicoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a monicoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Monicoin\n";
        else
            optionFile << strprintf("Name=Monicoin (%s)\n", chain);
