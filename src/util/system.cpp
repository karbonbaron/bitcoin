...
const char * const MONICOIN_CONF_FILENAME = "monicoin.conf";
const char * const MONICOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/monicoin/monicoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //monicoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", MONICOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Monicoin
    // macOS: ~/Library/Application Support/Monicoin
    // Unix-like: ~/.monicoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Monicoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Monicoin";
#else
    // Unix-like
    return pathRet / ".monicoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", MONICOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Monicoin Core copyright is not removed by accident
    if (copyright_devs.find("Monicoin Core") == std::string::npos) {
